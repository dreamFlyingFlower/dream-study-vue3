...
import { saveToken, readToken, delToken } from "../utils/localStorageUtils";
...
const errorHandle = (status: number, other: any) => {
  ...
  switch (status) {
    ...
    case 400:
      // console.log(other);
      // console.log("1231325456");
      ElMessage.error(other.message);
      break;
    // 401: 未登录
...
const instance = axios.create({
  ...
  baseURL: "http://172.30.252.32:3004",
  //     ? "测试"
  //     : "正式",
  // 表示跨域请求时是否需要使用凭证
...
instance.interceptors.request.use(
  (config: any) => {
    if (config.url !== "/ml/system/user/profile") {
    ...
    // console.log(config);
    if (config.url !== "/ml/system/login") {
      config.headers.Authorization = readToken();
      config.headers["X-Auth-Tenant"] = getTenantCode();
      config.headers["X-Auth-Token"] = getToken();
    }
    if (
      config.url == "/ml/system/user/export" ||
      config.url == "/ml/InspectionTemplate/excel"
    ) {
    ...
    if (response) {
      ...
      if (config && RETRY_COUNT) {
        ...
        if (config.__retryCount >= RETRY_COUNT) {
          return Promise.reject(response || { ElMessage: error.ElMessage });
        }
        // 增加重试计数
        config.__retryCount++;
        // 创造新的Promise来处理指数后退
