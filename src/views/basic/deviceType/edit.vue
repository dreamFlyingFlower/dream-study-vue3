...
import { DeviceTypeEntity } from "../../../types/deviceType";
...
// 声明表单数据
const formData = reactive<DeviceTypeEntity>({
  rowId: null,
  typeCode: null,
  typeName: null,
  superiorCode: null,
...
const checkSuperiorCode = (rule: any, value: any, callback: any) => {
  if (!value || value.trim() === "") {
    ...
    if (formData.typeCode === formData.superiorCode) {
      callback(new Error("上级设备类型不能是自己,请重新选择!"));
    } else {
      callback();
    }
  }
...
const editRules = reactive({
  ...
  typeName: [{ required: true, trigger: "blur", tip: "设备类型名称重复,请重新输入!", validator: hasTypeName }],
  typeCode: [{ required: true, trigger: "blur", tip: "设备类型编码重复,请重新输入!", validator: hasTypeCode }]
});
