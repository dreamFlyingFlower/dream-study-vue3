    ...
    <div class="IndexRight">
      <el-card shadow="hover">
        <div class="InquireWrap">
          <div class="InquireList">
            <el-input size="small" v-model="moduleData.typeName" placeholder="设备类型名称" @keyup.enter="getList"
              clearable />
      ...
      <el-table class="spareTable" :data="moduleData.tableDatas" ref="tableDatasRef" empty-text="暂无数据"
        ...
        <el-table-column :show-overflow-tooltip="true" align="center" min-width="100" prop="superiorCode"
          label="上级类型编码">
          <template v-slot:default="scope">
            <span v-text="scope.row.superiorCode &&	scope.row.superiorCode !== '0'? scope.row.superiorCode: '无'"></span>
          </template>
        </el-table-column>
        <el-table-column :show-overflow-tooltip="true" align="center" min-width="100" prop="superiorName"
          label="上级类型名称">
          <template v-slot:default="scope">
            <span v-text="scope.row.superiorName? scope.row.superiorName: '设备类型'"></span>
          </template>
        </el-table-column>
...
import type { DeviceTypeModule, DeviceTypeEntity } from "../../../types/deviceType";
import api from "../../../api/deviceType";
...
// 页面数据
const moduleData = reactive<DeviceTypeModule>({
  tableDatas: [],
  treeData: [],
  typeName: "",
  editDialog: false,
...
// 点选表格前的勾选框操作
const multipleSelection = ref<DeviceTypeEntity[]>([]);
...
// 点击表格多选框操作
const handleSelectionChange = (val: DeviceTypeEntity[]) => {
  multipleSelection.value = val;
}

// 删除单条数据
const removeData = (typeCode: string) => {
  ElMessageBox.confirm('此操作将删除该设备类型以及所有子类型,是否继续?', '提示', {
