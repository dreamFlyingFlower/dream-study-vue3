<template>
  <div class="IndexWrap">
      ...
      <div class="InquireWrap">
        <div class="InquireList">
          ...
          <el-select size="small" clearable v-model="stockInfo.type" placeholder="类型">
            ...
            <el-option label="盘点入库" value="盘点入库"> </el-option>
            <el-option label="领用返还" value="领用返还"> </el-option>
          </el-select>
          <el-select
            size="small"
            clearable
          ...
          <el-input
            ...
            clearable
          />
          <el-button size="small" @click="getInputwarehouseList('default')"
            >查询</el-button
          >
        </div>
        <div>
          <el-button size="small" type="primary" @click="newAddSpare">添加入库</el-button>
          <el-button size="small" type="info" @click="getInputwarehouseList"
            >导出</el-button
          >
        </div>
      </div>
    ...
    >
      ...
      <el-table-column label="操作" min-width="80" align="center">
        <template v-slot:default="scope">
          <el-button type="text" size="small" @click.stop="warehousingDetails(scope.row)"
            >详细</el-button
          >
          <el-button type="text" size="small" @click.stop="editWarehousing(scope.row)"
            >修改</el-button
          >
          <el-button type="text" size="small" @click.stop="delWarehousing(scope.row.id)"
            >删除</el-button
          >
        </template>
      </el-table-column>
  ...
  <AddToStock
    :dialog="stockInfo.warehousingDialog"
...
import { stockTyoe } from "../../types/storehouse";
const stockInfo = reactive<stockTyoe>({
  stockTable: [],
  detailList: [],
  warehouseList: [],
  selcetedIndex: 0,
...
onMounted(async () => {
  await getInputwarehouseList("default");
...
// 查询入库信息列表
const getInputwarehouseList = (defaultType: string = "") => {
  api.article
    .getInputwarehouse({
...
const newAddSpare = () => {
  ...
  stockInfo.rowInfo = "";
  stockInfo.warehousingDialog = true;
};
// 关闭弹窗
const colseDialog = () => {
  stockInfo.warehousingDialog = false;
  getInputwarehouseList();
};
// 详情
const warehousingDetails = (row: any) => {
...
// 删除
const delWarehousing = (Id: number) => {
